...
# 🍷 Hybrid Wine Recommendation System
...
- `model.py`: Trains two models: Collaborative Filtering (SVD) and Content-Based Filtering (Random Forest) using persona–item match features. Evaluates prediction accuracy and analyzes cold-start recommendations. Includes a hybrid recommend function for user persona–based Top-N recommendations from the test pool.
...
## 🛠 Used Libraries (not taught in class)
- **scikit-learn**
...
2. **Synthetic Ratings** (`create_wine_ratings.py`): Creates user–wine ratings using persona rules and bridge matching. Ratings distributed using soft weighting for realistic bias and cold-start simulation.
